...
def demo_mcp_with_permissions():
    ...
    mcp_agent.register_tool(
        "web_search", example_web_search_tool, PermissionNames.MCP_WEB_SEARCH
    )
    mcp_agent.register_tool(
        "file_write", example_file_write_tool, PermissionNames.MCP_FILE_WRITE
    )
    mcp_agent.register_tool(
        "system_command",
        example_system_command_tool,
        PermissionNames.MCP_SYSTEM_COMMAND,
    )
    ...
    test_cases = [
        {
            ...
            "task": {"tool_name": "web_search", "description": "搜索机器学习相关信息"},
        },
        {
            "name": "文件写入（HIGH - 需要人工审批）",
            "task": {"tool_name": "file_write", "description": "写入配置文件"},
        },
        {
            "name": "系统命令（CRITICAL - 需要人工审批）",
            "task": {"tool_name": "system_command", "description": "执行系统更新"},
        },
    ]
