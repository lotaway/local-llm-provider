...
print(
    "python arch", platform.architecture()[0], "pointer bits", struct.calcsize("P") * 8
)
...
for p in os.environ.get("PATH", "").split(os.pathsep):
    ...
    if os.path.exists(f):
        hip.append(f)
sys32 = os.path.join(
    os.environ.get("SYSTEMROOT", "C:\\Windows"), "System32", "amdhip64.dll"
)
    ...
    try:
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                'import torch; print(getattr(torch.version,"cuda",None)); print(torch.cuda.is_available())',
            ],
            capture_output=True,
            text=True,
        )
