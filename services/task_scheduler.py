        ...
        try:
            ...
            if MONGO_USER and MONGO_PASSWORD:
                if "@" not in mongo_uri:
                    ...
                    mongo_uri = (
                        f"mongodb://{MONGO_USER}:{MONGO_PASSWORD}@{host_port}{rest}"
                    )
    ...
    async def register_root_task(self):
        ...
        if not existing:
            ...
            self._collection.update_one(
                {"task_id": task_id}, {"$set": task}, upsert=True
            )
    ...
    async def _execute_task(self, task_data: Dict[str, Any]):
        ...
        if not backend_globals.agent_runtime:
            try:
                ...
                from clients.agents.context_storage import create_context_storage
        ...
        self._collection.update_one(
            {"task_id": task_id}, {"$set": {"last_run": time.time()}}
        )
                ...
                {"task_id": task_id},
                {
                    "$push": {
                        "history": {
                            ...
                            "summary": str(state.final_result)[:500]
